...
@available(*, deprecated, message: "ENMBadgedBarButtonItem will soon become unavailable. Please use the updated BadgedBarButtonItem class name")
...
open class BadgedBarButtonItem: UIBarButtonItem {
    ...
    @IBInspectable open var badgeValue: Int  = 0 {
        didSet {
            ...
            if (badgeLabel.superview != nil) {
                updateBadgeValue(shouldAnimateBadge)
            } else {
    ...
    */
    open var badgeProperties: BadgeProperties
    
    open var isShoppingButtonItem: Bool = false
    ...
    fileprivate func performStoryboardInitalizationSetup(with frame: CGRect, title: String? = nil, image: UIImage?) {
        let btn = createInternalButton(frame, title: title, image: image)
        self.customView = btn
...
public extension BadgedBarButtonItem {
    ...
    */
     ...
     */
    func createInternalButton(_ frame: CGRect,
                                   title: String? = nil,
    ...
    func calculateUpdatedBadgeFrame(usingFrame frame: CGRect) {
        badgeProperties.originalFrame.origin.x = (frame.size.width - 1) - (badgeLabel.frame.size.width/2) - 10
    }
}
...
fileprivate extension BadgedBarButtonItem {
    ...
    func updateBadgeValue(_ animated: Bool) {
